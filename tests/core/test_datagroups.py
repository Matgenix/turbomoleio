...
from turbomoleio.core.datagroups import (
    DataGroups,
    cleanup_string,
...
from turbomoleio.testing import temp_dir, touch_file
...
def test_cleanup():
    ...
    with pytest.raises(
        expected_exception=ValueError, match=r'^No dollar \("\$"\) sign in the string.$'
    ):
        cleanup_string(string=sample_string, cleanup_types=None)

    # Testing : Invalid type of cleanup
    sample_string = "$title"
    with pytest.raises(
        expected_exception=ValueError,
        match=r'^Cleanup of type "UNKNOWN" is not a valid ' r"type.$",
    ):
        cleanup_string(string=sample_string, cleanup_types=["UNKNOWN"])

    # Testing : Characters preceding a $ sign
    sample_string = "$title\na$dummy"
    with pytest.raises(
        expected_exception=ValueError,
        match=r"^Some character\(s\) are preceding a " r'dollar \("\$"\) sign.$',
    ):
        cleanup_string(string=sample_string, cleanup_types=None)

    # Testing : inline comments are acceptable
    sample_string = "$title\na#this is a comment line\n$end"
    new_string = cleanup_string(string=sample_string, cleanup_types=None)
    ...
    new_string = cleanup_string(
        string=sample_string, cleanup_types=["BEFORE_FIRST_DOLLAR"]
    )
    ...
    new_string = cleanup_string(string=sample_string, cleanup_types=["BLANK_LINES"])
    assert new_string == "1\n2\n3\n$dummy abcd\n$key2 var2\n    data\n$end\n"
    ...
    new_string = cleanup_string(
        string=sample_string, cleanup_types=["LEADING_SPACES_DOLLAR"]
    )
    assert (
        new_string == "1\n2\n3\n$dummy abcd\n  \n"
        "$key2 var2\n    data\n\n\n\n\n$end\n"
    ...
    new_string = cleanup_string(
        string=sample_string, cleanup_types=["LEADING_SPACES_HASH"]
    )
    assert (
        new_string == "1\n2\n3\n#this is a comment\n  \n$key2 var2\n"
        "    data\n\n\n\n\n"
    ...
    sample_string = "$title\n" "$coord  file=coord\n"
    with pytest.raises(
        expected_exception=ValueError, match=r'^No "\$end" in the string.$'
    ):
...
def test_remove_comments():
    ...
    sample_string = (
        "first line #this inline comment "
        "should not be removed here\n"
        "#this line should be removed\n"
        "$first dollar sign\n"
    )
    new_string = remove_comments(string=sample_string, comment_types=["HASH_START"])
    assert (
        new_string == "first line #this inline comment should not be "
        "removed here\n$first dollar sign\n"
    )
    ...
    with pytest.raises(
        expected_exception=ValueError,
        match=r'^Comment of type "UNKNOWN" is not a ' r"valid type.$",
    ):
        remove_comments(string=sample_string, comment_types=["UNKNOWN"])
    ...
    new_string = remove_comments(string=sample_string, comment_types=["DUMMY"])
    assert new_string == "1\n2\n3\n$key1\n$key2 var2\n    data\n"
    ...
    new_string = remove_comments(string=sample_string, comment_types=["AFTER_END"])
    assert new_string == "1\n2\n3\n$dummy abcd\n\n$key2 var2\n    data\n$end\n"
    ...
    new_string = remove_comments(string=sample_string, comment_types=["HASH_INLINE"])
    assert (
        new_string == "1\n2\n3\n"
        "#This hash starting line should not be removed\n"
        "$dummy abcd \n\n"
...
def test_split_string_to_dg_list():
    ...
    dg_list = split_string_to_dg_list(
        sample_string, cleanup_types=[], remove_comment_types=[]
    )
...
def test_remove_dg_from_list():
    ...
    dg_list = split_string_to_dg_list(sample_string)
    new_dg_list = remove_dg_from_list(
        dg_to_remove="operating", dg_list=dg_list, strict=True
    )
    assert "$operating\tsystem unix\n" not in new_dg_list

    # Testing not strict removal
    new_dg_list = remove_dg_from_list(
        dg_to_remove="$opera", dg_list=dg_list, strict=False
    )
    assert "$operating\tsystem unix\n" not in new_dg_list

    new_dg_list = remove_dg_from_list(dg_to_remove="scf", dg_list=dg_list, strict=False)
    assert "$operating\tsystem unix\n" in new_dg_list


def test_compare_datagroup_string():
    dg1 = (
        "$forceupdate\n   ahlrichs numgeo=0  mingeo=3 maxgeo=4 modus=<g|dq> "
        "dynamic fail=0.3\n   threig=0.005  reseig=0.005  thrbig=3.0  "
        "scale=1.00  damping=0.0\n"
    )
    dg1_b = (
        "$forceupdate\n   ahlrichs numgeo=0  mingeo=3 maxgeo=4 modus=<g|dq> "
        "dynamic fail=0.3\n   threig=0.005  reseig=0.004  thrbig=3.0  "
        "scale=1.00  damping=0.0\n"
    )
    dg1_c = (
        "$forceupdate\n   xxxahlrichs numgeo=0  mingeo=3 maxgeo=4 modus=<g|dq> "
        "dynamic fail=0.3\n   threig=0.005  reseig=0.005  thrbig=3.0  "
        "scale=1.00  damping=0.0\n"
    )
...
def test_datagroups():
    ...
    # Testing : no input
    with pytest.raises(
        expected_exception=ValueError, match=r'^Both "string" and "dg_list" are None.$'
    ):
        DataGroups()
    ...
    dg = DataGroups.empty()
    invalid_name_regex = (
        r"^Data group should start with a letter and be "
        r'followed by alphanumeric characters, a space, "-", "_", "\(" or "\)".$'
    )
    with pytest.raises(expected_exception=ValueError, match=invalid_name_regex):
    ...
    with pytest.raises(
        expected_exception=RuntimeError,
        match=r"^Found multiple occurrences of data group " r'"\$scf".$',
    ):
...
def test_modify_data_group_options(test_data):
    cc = DataGroups.from_file(test_data / "control" / "control_test-energy")
    cc.modify_data_group_options(
        "dft", {"gridsize": "gridsize m5", "ntheta": "ntheta 30"}
    )
...
def test_show_data_group_option(test_data):
    cc = DataGroups.from_file(test_data / "control" / "control_test-energy")
...
def test_sdg_subfiles(test_data, delete_tmp_dir):
    with temp_dir(delete_tmp_dir):
        for fname in ["control", "coord", "energy", "mos", "basis"]:
            shutil.copy2(
                test_data / "control" / "{}_test-subfiles".format(fname),
                fname,
            )
        ...
        udef_bonds_dg = cc.sdg(
            data_group="user-defined bonds",
            show_from_subfile=True,
            raise_if_multiple_subfiles=False,
            raise_if_missing_subfile=False,
            raise_if_regular_and_subfile=False,
        ...
        assert mos_dg.startswith(
            "    scfconv=6   format(4d20.14)\n"
            "     1  a1     "
            "eigenvalue=-.98973943603420D+01"
            "   nsaos=6\n"
        )
        assert mos_dg.endswith(
            "-.11000018463806D+01-.60830569311504D+00"
            "0.13711445998604D+010.13421520432673D+01\n"
            "0.53804368223874D+00-.91129252301913D+00"
            "0.11101532709297D+01\n"
        )
        scfintunit_dg = cc.sdg(
            data_group="scfintunit",
            show_from_subfile=True,
            raise_if_multiple_subfiles=False,
            raise_if_missing_subfile=False,
            raise_if_regular_and_subfile=False,
        ...
        with pytest.raises(RuntimeError, match='Multiple "file=FILENAME" directives.'):
            cc.sdg(
                data_group="scfintunit",
                show_from_subfile=True,
        ...
        with pytest.raises(
            RuntimeError, match='File "gradient" for data group "grad" ' "is missing."
        ):
            cc.sdg(
                data_group="grad",
                show_from_subfile=True,
...
def test_show_subfile_fname(test_data):
    cc = DataGroups.from_file(test_data / "control" / "control_test-energy")
