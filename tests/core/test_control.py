...
from turbomoleio.testing import assert_MSONable, has_matplotlib, temp_dir
...
class TestEnergy:
    def test_energy_scf(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "energy_test-energy", "energy")
    ...
    def test_energy_mp2(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "energy_test-mp2", "energy")
            energy = Energy.from_file()
...
class TestGradient:
    def test_gradient(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "gradient_test-gradient", "gradient")
    ...
    def test_fail_parsing(self, test_data):
        with open(test_data / "control" / "gradient_test-gradient") as f:
            grad = f.read()
    ...
    def test_periodic(self, test_data, delete_tmp_dir):
        # 1 dimensional
        with temp_dir(delete_tmp_dir):
            shutil.copy2(
                test_data / "control" / "periodic_1D_C_chain.control",
                "control",
            )
            shutil.copy2(test_data / "control" / "periodic_1D_C_chain.energy", "energy")
            shutil.copy2(
                test_data / "control" / "periodic_1D_C_chain.gradient",
                "gradient",
            )
        ...
        with temp_dir(delete_tmp_dir):
            shutil.copy2(
                test_data / "control" / "periodic_2D_graphene.control",
                "control",
            )
            shutil.copy2(
                test_data / "control" / "periodic_2D_graphene.energy",
                "energy",
            )
            shutil.copy2(
                test_data / "control" / "periodic_2D_graphene.gradient",
                "gradient",
            )
            g = Gradient.from_file(filename="control")
        ...
        # 3 dimensional
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "periodic_3D_HC.control", "control")
            shutil.copy2(test_data / "control" / "periodic_3D_HC.energy", "energy")
            shutil.copy2(test_data / "control" / "periodic_3D_HC.gradient", "gradient")
            g = Gradient.from_file(filename="control")
...
class TestShells:
    def test_closed(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "control_test-subfiles", "control")
...
class TestControl(object):
    ...
    def test_energy(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            for fname in ["control", "energy"]:
                source = test_data / "control" / "{}_test-subfiles".format(fname)
    ...
    def test_remove_energy(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            shutil.copy2(test_data / "control" / "control_test-subfiles", "control")
            shutil.copy2(test_data / "control" / "energy_test-energy", "energy")
    ...
    def test_remove_energy_in_control(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
    ...
    def test_remove_gradient_in_control(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
    ...
    def test_get_charge(self, test_data, delete_tmp_dir):
    ...
    def test_get_subfiles_list(self, test_data):

        c = Control.from_file(test_data / "control" / "control_test-subfiles")
    ...
    def test_cpc(self, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):
            c_empty = Control.empty()
...
class TestDatagroupFunctions(object):
    ...
    def test_cpc(self, control_filepath, test_data, delete_tmp_dir):
        with temp_dir(delete_tmp_dir):

            os.makedirs("orig_dir")

            shutil.copy2(
                test_data / "control" / "control_test-subfiles",
                os.path.join("orig_dir", "control"),
            )
            shutil.copy2(
                test_data / "control" / "gradient_test-gradient",
