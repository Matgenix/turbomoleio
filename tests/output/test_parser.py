...
TESTS_CONFIGS_TM_VERSIONS = get_tests_configs_tm_versions(TEST_DATA)
...
def parser_and_dict(request, test_data):
    tm_version = request.param[0]
    tm_exec = request.param[1]
    test_name = request.param[2]
    path = test_data / "outputs" / tm_version / tm_exec / test_name
...
class TestParser:
    ...
    def test_get_split_jobex_parsers(self, test_data, tm_version):
        path = test_data / "outputs" / tm_version / "jobex" / "h2o_dscf" / "job.last"
        ...
        path = test_data / "outputs" / tm_version / "jobex" / "no3_ridft" / "job.last"
        p = Parser.from_file(path)
        jp = p.get_split_jobex_parsers()
