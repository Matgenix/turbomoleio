...
class MoleculeSystem(BaseSystem):
    ...
    def __init__(
        self, molecule, int_def=None, frozen_indices=None, user_defined_bonds=None
    ):
        ...
        self.user_defined_bonds = (
            set(user_defined_bonds) if user_defined_bonds else set()
        )
        if not isinstance(molecule, Molecule):
            raise ValueError(
                "A Molecule object should be provided for molecule systems."
            )
        super().__init__(
            molecule_or_structure=molecule, frozen_indices=frozen_indices, periodicity=0
        )
    ...
    def from_string(cls, string, fmt="coord"):
        ...
        Could be the string of a coord file or any format supported by
        pymatgen Molecule.
        ...
        if fmt == "coord":
            ...
            if int_def_str:
                ...
                for line in int_def_str.splitlines():
                    lstrip = line.strip()
                ...
                # will split in 3 groups based on the presence of the
                # digit plus k, f, d or i at the beginning of the line.
                ...
                for line in user_def_bonds_str.splitlines():
                    ...
                    for bond in line.split(","):
                        for separator in ("-", "|"):
                            ...
                            raise ValueError(
                                "Cannot parse user-defined bonds for line: {}".format(
                                    line
                                )
                            )
    ...
    def _check_index(self, indices_list):
        ...
        This helper method determines if one of the indices exceeds the number of
        ...
        if any(n < 0 or n >= n_sites for n in indices_list):
            raise ValueError(
                "One of the indices representing the atoms is negative or "
    ...
    def add_distance(
        ...
        Allows to set linear combinations of the coordinate. If a single value
        is given to the arguments it will be a simple internal coordinate.
        If a list of atoms and values are given this will produce a linear
        combination of those coordinates.

        Args:
            atom1 (int or list): index (0-based) or list on indices of
                the first atom(s).
            atom2 (int or list): index (0-based) or list on indices of
                the second atom(s).
        ...
        self.int_def.append(
            Distance(status=status, indices=indices, weights=weights, value=value)
        )
    ...
    def add_bond_angle(
        ...
        Allows to set linear combinations of the coordinate. If a single value is
        given to the arguments it will be a simple internal coordinate. If a list of
        atoms and values are given this will produce a linear combination of
        those coordinates.

        Args:
            atom1 (int or list): index (0-based) or list on indices of the
                first atom(s).
            atom2 (int or list): index (0-based) or list on indices of the
                second atom(s).
            ...
            weights (float or list): weight or list of weights in case of linear
                ...
                If None will be set to a list of 1.0 with the same length as atoms.
            status (str): the status of the coordinate, can be "k", "f", "d" and "i".
            add_user_def_bonds (bool): if True the pair of atoms will be added to
                the used defined bonds
        """
        if np.ndim(atom1) == 0:
            atom1 = [atom1]
        if np.ndim(atom2) == 0:
        ...
        self.int_def.append(
            BondAngle(status=status, indices=indices, weights=weights, value=value)
        )
    ...
    def add_dihedral(
        ...
        Allows to set linear combinations of the coordinate. If a single value is
        given to the arguments it will be a simple internal coordinate. If a list
        of atoms and values are given this will produce a linear combination of
        those coordinates.
        ...
        Args:
            ...
            atom2 (int or list): index (0-based) or list on indices of the
                second atom(s).
            atom3 (int or list): index (0-based) or list on indices of the
                third atom(s).
            atom4 (int or list): index (0-based) or list on indices of the
                fourth atom(s).
            value (float): value of the distance.
            weights (float or list): weight or list of weights in case of linear
                combinations of coordinates. Should have the same length as the atoms.
        ...
        self.int_def.append(
            DihedralAngle(status=status, indices=indices, weights=weights, value=value)
        )
    ...
    def get_int_def_inconsistencies(self, atol=1e-4, ltol=1e-4):
        ...
        This returns a list of the internal coordinates whose values are not
        consistent with the values obtained from the molecule object.
    ...
    def as_dict(self):
        ...
        d = {
            ...
            "int_def": [i.as_dict() for i in self.int_def],
            "frozen_indices": sorted(self.frozen_indices),
            "user_defined_bonds": sorted(self.user_defined_bonds),
        }
...
class InternalDefinition(abc.ABC, MSONable):
    ...
    # coord_type is a string describing the type of coordinate.
    # could be "length" or "angle".
    ...
    def __init_subclass__(cls, **kwargs):
        """Check the presence of implemented subclass attributes."""
        super().__init_subclass__(**kwargs)

        if cls.coord_str is NotImplemented:
            raise NotImplementedError(
                "Subclasses should implement the class attribute coord_str"
            )

        if cls.n_atoms is NotImplemented:
            raise NotImplementedError(
                "Subclasses should implement the class attribute n_atoms"
            )

        if cls.coord_type is NotImplemented:
            raise NotImplementedError(
                "Subclasses should implement the class attribute coord_type"
            )
    ...
    def __init__(self, status, indices, weights=None, value=None):
        ...
        if status not in ("k", "f", "d", "i"):
            raise ValueError(
                "Unsupported internal definition status: {}".format(status)
            )
    ...
    def get_subclass_from_str(coord_str):
        ...
        raise ValueError(
            "Could not find a subclass with coord_str matching {}".format(coord_str)
        )
    ...
    def to_string(self, index=1):
        ...
        """
        first_line = "{} {} {} {} ".format(
            index, self.status, self.weights[0], self.coord_str
        )
        ...
        if len(self.indices) > 1:
            for w, ind in zip(self.weights[1:], self.indices[1:]):
                line = "    {} {} ".format(w, self.coord_str) + " ".join(
                    str(i + 1) for i in ind
                )
                lines.append(line)
    ...
    def is_valid(self, molecule, tol=1e-4):
        ...
        If the value of this internal coordinate is not None, it checks if it is
        consistent with the positions of the atoms in the given molecule.
...
class Distance(InternalDefinition):
    ...
    def compute_value(self, molecule):
        ...
        for weight, indices in zip(self.weights, self.indices):
            tot_val += (
                weight * molecule.get_distance(indices[0], indices[1]) * ang_to_bohr
            )
...
class DihedralAngle(InternalDefinition):
    ...
    def compute_value(self, molecule):
        ...
        tot_val = 0
        for weight, indices in zip(self.weights, self.indices):
            tot_val += weight * molecule.get_dihedral(
    ...
    def __str__(self):
        """Get a string representation of the Dihedral Angle."""
        return (
            "Dihedral Angle between the planes defined by atoms with indices {}".format(
                self.indices
            )
        )
...
class InverseDistance(InternalDefinition):
    ...
    def compute_value(self, molecule):
        ...
        for weight, indices in zip(self.weights, self.indices):
            tot_val += weight / (
                molecule.get_distance(indices[0], indices[1]) * ang_to_bohr
            )
    ...
    def __str__(self):
        """Get a string representation of the Inverse Distance."""
...
class OutOfPlaneAngle(InternalDefinition):
    ...
    def __str__(self):
        """Get a string representation of the Out of plane angle."""
...
class CollinearBendingAngle(InternalDefinition):
    ...
    def __str__(self):
        """Get a string representation of the Collinear bending angle."""
...
class PerpendicularBendingAngle(InternalDefinition):
    ...
    def __str__(self):
        """Get a string representation of the Perpendicular bending angle."""
        return "Perpendicular bending angle for atoms with indices {}".format(
            self.indices
        )
