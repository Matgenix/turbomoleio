...
class TestMoleculeSystem:
    ...
    def test_distance_no_val(self, molecule, control_filepath, delete_tmp_dir):
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+stre\s+1\s+2\s+val=\s*2\.05", redundant
            )

    def test_distance_linear_combination(
        self, molecule, control_filepath, delete_tmp_dir
    ):
        ms = MoleculeSystem(molecule)
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+stre\s+1\s+2\s+"
                r"val=\s*2\.70\d+\s+1\.0+\s+stre\s+2\s+3\s*$",
                redundant,
    ...
    def test_bond_angle(self, molecule, control_filepath, delete_tmp_dir):
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+bend\s+1\s+2\s+3\s+val=\s*30", redundant
            )
    ...
    def test_dihedral_angle(self, molecule, control_filepath, delete_tmp_dir):
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+tors\s+1\s+2\s+3\s+4\s+val=\s*30", redundant
            )
    ...
    def test_inverse_distance(self, molecule, control_filepath, delete_tmp_dir):
        ms = MoleculeSystem(molecule)
        ms.int_def.append(
            InverseDistance(indices=[0, 1], value=0.5, weights=[1.0], status="f")
        )
        assert ms.has_inconsistencies()
    ...
    def test_out_of_plane(self, molecule, control_filepath, delete_tmp_dir):
        ms = MoleculeSystem(molecule)
        ms.int_def.append(
            OutOfPlaneAngle(indices=[[0, 1, 2, 3]], value=-25, weights=None, status="f")
        )
        assert ms.has_inconsistencies()
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+outp\s+1\s+2\s+3\s+4\s+val=\s*-25", redundant
            )
    ...
    def test_linc(self, molecule, control_filepath, delete_tmp_dir):
        ms = MoleculeSystem(molecule)
        ms.int_def.append(
            CollinearBendingAngle(indices=[[0, 1, 2, 3]], value=80, status="f")
        )
        assert ms.has_inconsistencies()
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+linc\s+1\s+2\s+3\s+4\s+val=\s*80", redundant
            )
    ...
    def test_linp(self, molecule, control_filepath, delete_tmp_dir):
        ms = MoleculeSystem(molecule)
        ms.int_def.append(
            PerpendicularBendingAngle(indices=[[0, 1, 2, 3]], value=20, status="f")
        )
        assert ms.has_inconsistencies()
        ...
        with temp_dir(delete_tmp_dir):
            ...
            redundant = dg.sdg("redundant", strict=True)
            assert re.search(
                r"\d+\s+f\s+1\.0+\s+linp\s+1\s+2\s+3\s+4\s+val=\s*20", redundant
            )
